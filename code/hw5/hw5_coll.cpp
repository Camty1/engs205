...
int main(int argc, char** argv) {
    ...
    fftw_plan u_dbl_prime_inverse = fftw_plan_dft_1d(N, (fftw_complex*) u_dbl_prime_tilde.data(), (fftw_complex*) u_dbl_prime.data(), FFTW_BACKWARD, FFTW_ESTIMATE);
    ...
    for (int l = 0; l < timesteps; l++) {
        ...
        fftw_execute(u_dbl_prime_inverse);

        for (int j = 0; j < N; j++) {
            u[j] = u[j] / (double) N;
            u_prime[j] = u_prime[j] / (double) N;
            u_dbl_prime[j] = u_dbl_prime[j] / (double) N;
        }
        ...
        // Step time
        for (int j = 0; j < N; j++) {
            u[j] = u[j] + dt * (v * u_dbl_prime[j] - u_prime[j]);
        }
