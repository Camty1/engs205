...
elem_bc_type = [2 * ones(1, 24), ones(1, 24)]';


x = [linspace(1, -1, 25), cos(theta(2:end-1))]';
y = [zeros(1, 25), sin(theta(2:end-1))]';
node_list = [nodes, x, y];

node_bc_type_old = [1; 2 * ones(23,1); ones(24, 1)];
...
elem_bc_type = [2 * ones(1, 24), zeros(1, 24)]';
elem_bc_type_2 = 2 * sin(pi * elem_midpoints(:, 1) / 2);
elem_bc_val = zeros(48, 1);
...
writematrix([nodes, node_bc_type_old, node_bc_val_old], [DEF_DIR '/hw2.bcs'], 'FileType', 'text');

% New, BEMPAK like formulation
writematrix([node_list, node_bc_type_new, node_bc_val_new], [DEF_DIR '/hw2.nodb'], 'FileType', 'text');
writematrix([elem_list, ones(48, 1), zeros(48, 1), elem_bc_type, elem_bc_val], [DEF_DIR '/hw2.eleb'], 'FileType', 'text');
